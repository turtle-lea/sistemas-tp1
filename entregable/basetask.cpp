...
vector<ptsk> tasks_load(const char* filename) {
	...
	while(getline(f, s)) { l++;
		...
		for(int j=0; j<times; j++) ts.push_back(ptsk(ti.first, params, starttm));
	}
	return ts;
}

ptsk::ptsk(TaskBase* vtsk, const std::vector<int>& vprms, unsigned int vstart) : tsk(vtsk), prms(vprms), start(vstart) {}
ptsk::ptsk(void) {}
